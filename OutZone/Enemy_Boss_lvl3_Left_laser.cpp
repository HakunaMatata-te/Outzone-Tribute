	...
	if (App->level_3->boss_start){
		...
		if (stage == LMOVEMENT::LASER_CREATION){
			if (SDL_GetTicks() - sTimer > 25){
				App->particles->AddParticle(App->particles->boss_left_shot_apear, position.x + 25, position.y + 29, COLLIDER_LASER_LEFT);
				sTimer = SDL_GetTicks();
			}
			if (SDL_GetTicks() - Timer > 1620){
				App->particles->AddParticle(App->particles->boss_left_sparkle, 226, App->collision->returny(416, App->level_3->background.h), COLLIDER_LASER_LEFT);
				Timer = SDL_GetTicks();
				stage = LASER_MOVE;
			}
		}
			...
			for (uint i = 0; i < MAX_ACTIVE_PARTICLES; i++){
				if (App->particles->active[i] != nullptr){
					if (App->particles->active[i]->life == 13026){ //only long shot particles should have this life.
						...
						App->particles->AddParticle(App->particles->boss_left_long_shot, App->particles->active[i]->position.x, App->particles->active[i]->position.y, COLLIDER_LASER_LEFT);
